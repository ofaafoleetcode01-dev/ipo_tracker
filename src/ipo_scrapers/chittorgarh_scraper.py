...
class ChittorgarhScraper(object):
    def __init__(self):
        self.base_url = "https://www.chittorgarh.com"
        self.year = 2026

    def scrape_ipos(self) -> list[IPOSubscription]:
        """Main scraping entry point. Returns a list of IPOSubscription objects."""
        base_url = self.base_url
        year = self.year

        ipos_meta = self._fetch_mainboard_ipos(base_url, year)
        if not ipos_meta:
        ...
        for meta in ipos_meta:
            try:
                details = self._fetch_subscription_details(base_url, meta["slug"], meta["ipo_id"])
            except Exception:
    ...
    def _parse_close_date_from_badge(self, badge_text: str, year: int) -> Optional[date]:
    ...
    def _extract_slug_and_id(self, href: str) -> tuple[str, str]:
    ...
    def _fetch_mainboard_ipos(self, base_url: str, year: int) -> list[dict]:
        ...
        for row in tbody.find_all("tr"):
            ...
            href = link.get("href", "")
            slug, ipo_id = self._extract_slug_and_id(href)
            ...
            for span in spans:
                ...
                close_date = self._parse_close_date_from_badge(span_text, year)
                if close_date:
                    break
    ...
    def _fetch_subscription_details(self, base_url: str, slug: str, ipo_id: str) -> dict:
        ...
        for table in tables:
            ...
            if "subscription (times)" in header_texts or "subscription(times)" in " ".join(header_texts):
                self._parse_subscription_table(table, result)
            elif "investor category" in header_texts and "subscription" in " ".join(header_texts):
                self._parse_subscription_table(table, result)

        self._parse_dates_table(tables, result)
        self._parse_detailed_subscription(tables, result)

        return result


    def _parse_subscription_table(self, table: Tag, result: dict) -> None:
        ...
        for row in tbody.find_all("tr"):
            ...
            value = self._parse_float(tds[1].get_text(strip=True))
    ...
    def _parse_detailed_subscription(self, tables: list[Tag], result: dict) -> None:
        ...
        for table in tables:
            ...
            for row in tbody.find_all("tr"):
                ...
                if snii_idx is not None and snii_idx < len(tds):
                    result["snii"] = self._parse_float(tds[snii_idx].get_text(strip=True))
                if bnii_idx is not None and bnii_idx < len(tds):
                    result["bnii"] = self._parse_float(tds[bnii_idx].get_text(strip=True))
                break


    def _parse_dates_table(self, tables: list[Tag], result: dict) -> None:
